...
import {
    ...
    cilLocationPin,
    cilCalendar,
    cilCheckCircle, // Import check circle icon
    cilWarning,     // Import warning icon
    cilXCircle      // Import x circle icon
} from "@coreui/icons";
    ...
    <CTableRow>
        ...
        <CTableHeaderCell className="bg-body-tertiary">Actions</CTableHeaderCell>
            ...
            {agreements.map((agreement, index) => {
                    ...
                    return (
                        <CTableRow key={agreement._id}> {/* Correct key prop here - use agreement._id */}
                            ...
                            <CTableDataCell> {/* New Status Data Cell */}
                                <div className={`status-badge ${statusDetails.color}`}>
                                    {statusDetails.text}
                                    {statusDetails.icon && (
                                    ...
                                    <CDropdownMenu>
                                          {userPermissions?.editAgreement && (
                                             ...
                                             <CDropdownItem
                                                ...
                                                agreement.documents.map((doc, index) => ( // Keep index for now, but we'll use doc as key
                                                    <CDropdownItem
                                                        key={doc} // Use the document file path (doc) as the key - assuming it's unique
...
AgreementTable.propTypes = {
    agreements: PropTypes.arrayOf(
        PropTypes.shape({
            _id: PropTypes.string.isRequired, // PropType is already CORRECT - using _id
