...
const ClearanceTable = ({
    ...
    return [...clearances].sort((a, b) => {
      const aKey =
        a[sortConfig.key] && typeof a[sortConfig.key] === 'object'
          ? a[sortConfig.key]?.tenantName || a[sortConfig.key]?.name || ''
          : a[sortConfig.key] || ''
      const bKey =
        b[sortConfig.key] && typeof b[sortConfig.key] === 'object'
          ? b[sortConfig.key]?.tenantName || b[sortConfig.key]?.name || ''
          : b[sortConfig.key] || ''
  ...
  const debouncedSearch = useCallback(
    debounce((term) => {
      dispatch(
        fetchClearances({ page: 1, limit: itemsPerPage, search: term, status: statusFilter }),
      )
    }, 500),
  ...
  const confirmApprove = async () => {
    ...
    try {
      await dispatch(
        updateClearance({
          id: clearanceToApprove._id,
          clearanceData: { ...clearanceToApprove, status: 'Approved' },
        }),
      ).unwrap()
  ...
  const confirmReject = async () => {
    ...
    try {
      await dispatch(
        updateClearance({
          id: clearanceToApprove._id,
          clearanceData: { ...clearanceToApprove, status: 'Rejected' },
        }),
      ).unwrap()
  ...
  return (
    <div>
      ...
      />
      <CModal visible={approveModalVisible} onClose={handleCloseApproveModal} size="sm">
        <CModalHeader onClose={handleCloseApproveModal}>
          <CModalTitle>Approve or Reject Clearance Request</CModalTitle>
        </CModalHeader>
        <CModalBody>Are you sure you want to proceed with this request?</CModalBody>
        <CModalFooter>
