...
const ClearanceTable = ({
    ...
    const [clearanceToPass, setClearanceToPass] = useState(null);
    const [statusFilter, setStatusFilter] = useState('');
    const [role, setRole] = useState(null);
    const [passModalVisible, setPassModalVisible] = useState(false); // New Modal
    ...
    const handlePass = (clearance) => {
        setClearanceToPass(clearance);
        setPassModalVisible(true); // Open Pass modal
    };

    const handleDeleteItem = async (clearance) => {
        if (!clearance || !clearance._id) {
            toast.error('No clearance request selected for deletion');
            return;
        }
    
        try {
            await dispatch(deleteClearance(clearance._id)).unwrap();
    ...
    const confirmPass = async (result) => {
        if (!clearanceToPass?._id) {
            toast.error('No clearance selected to pass/fail.');
            return;
        }
        try {
            // Dispatch update action with inspection status
            await dispatch(updateClearance({ id: clearanceToPass._id, clearanceData: { inspectionStatus: result } })).unwrap();
            toast.success(`Clearance inspection status updated to ${result}.`);
            dispatch(fetchClearances({ page: currentPage, limit: itemsPerPage, search: searchTerm, status: statusFilter }));
           ...
           <ClearanceTableData
            ...
            >
             ...
             <CModal
                ...
                size="sm"
            >
                <CModalHeader onClose={handleClosePassModal}>
                    <CModalTitle>Update Inspection Status</CModalTitle>
                </CModalHeader>
                <CModalBody>
                    Select the inspection result:
                    <div className="d-flex justify-content-around mt-3">
                        <CButton color="success" onClick={() => confirmPass("Passed")}>
                            Pass
                        </CButton>
                        <CButton color="danger" onClick={() => confirmPass("Failed")}>
                            Fail
                        </CButton>
                    </div>
                </CModalBody>
                <CModalFooter>
                    <CButton color="secondary" onClick={handleClosePassModal}>
                        Cancel
                    </CButton>
                </CModalFooter>
            </CModal>
