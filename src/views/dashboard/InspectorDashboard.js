...
const InspectorDashboard = () => {
    ...
    // --- Helper function to format and display value safely ---
    const displayValue = (value) => {
        if (value === undefined || value === null) {
            return '---'; // Display placeholder if value is not available
        }
        return value.toString(); // Convert to string to ensure display
    };
                ...
                <CCol sm={6} lg={3}>
                    <SummaryCard>
                        <div className="summary-content">
                            <div className="summary-icon"><CIcon icon={cilCheckCircleIcon} height={36} style={{ color: colors.success }} />1</div>
                            <LabeledValue label="Completed Inspections" value={displayValue(inspectionStats.completedInspections)} />
                        </div>
                    </SummaryCard>
                </CCol>
                <CCol sm={6} lg={3}>
                    <SummaryCard>
                        <div className="summary-content">
                            ...
                            <LabeledValue label="Pending Inspections" value={displayValue(inspectionStats.pendingInspections)} />
                        </div>
                    </SummaryCard>
                </CCol>
                <CCol sm={6} lg={3}>
                    <SummaryCard>
                        <div className="summary-content">
                            ...
                            <LabeledValue label="Scheduled Inspections" value={displayValue(inspectionStats.scheduledInspections)} />
                        </div>
                    </SummaryCard>
                </CCol>
                <CCol sm={6} lg={3}>
                    <SummaryCard>
                        <div className="summary-content">
                            ...
                            <LabeledValue label="Total Inspections" value={displayValue(inspectionStats.completedInspections + inspectionStats.pendingInspections)} />
                        </div>
                    </SummaryCard>
                </CCol>
            </CRow>
            <CRow className="mb-4">
                ...
                <CCol md={4}>
                    <MetricCard gradient="linear-gradient(45deg, #2196F3, #42A5F5)">
                        ...
                        <div className="metric-body">
                            <LabeledValue value={inspectionStats.scheduledInspections} />
                        </div>
                    </MetricCard>
                </CCol>
            </CRow>
                                ...
                                <CTableBody>
                                    {recentInspections.map((inspection, index) => (
                                        <CTableRow key={inspection.id}>
                                            ...
                                            <CTableDataCell style={{ fontSize: '0.9rem' }}>{inspection.property}</CTableDataCell>
                                            <CTableDataCell style={{ fontSize: '0.9rem' }}>{inspection.date}</CTableDataCell>
                                            <CTableDataCell style={{ fontSize: '0.9rem' }}>
                                                {getStatusIcon(inspection.status)} {inspection.status}
                                            </CTableDataCell>
                                            <CTableDataCell style={{ fontSize: '0.9rem' }}>{inspection.inspector}</CTableDataCell>
                                        </CTableRow>
                                    ))}
                                </CTableBody>
                            </StyledTable>
