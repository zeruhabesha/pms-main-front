...
import propertyService from '../services/property.service'; // Adjust the path as needed
 ...
 extraReducers: (builder) => {
     ...
     .addCase(fetchProperties.rejected, (state, action) => {
         ...
         .addCase(updatePropertyPhotos.fulfilled, (state, action) => {
             ...
             state.error = null
             state.properties = state.properties.map((property) =>
                 property.id === action.payload.id ? { ...property, photos: action.payload.photos } : property
             )
             if (state.selectedProperty?.id === action.payload.id) {
